...
public class UserDAOImpl implements UserDAO{
	...
	public UserRole getUserRoleById(int id, Session ses) {
		...
		UserRole role = (UserRole) ses.createNativeQuery("SELECT * FROM system_userRoles WHERE system_userRole_id=" + id + "", UserRole.class).getSingleResult();	
	...
	public User selectById(int id) {
		...
		Session ses = HibernateUtil.getSession();
		User user = (User) ses.createNativeQuery("SELECT * FROM system_users WHERE system_user_id = " + id + "", User.class).getSingleResult();
	...
	public User selectByName(String name) {
		...
		Session ses = HibernateUtil.getSession();
		User user = (User) ses.createNativeQuery("SELECT * FROM system_users WHERE system_user_firstname = '" + name + "'", User.class).getSingleResult();
		System.out.println("Search complete! Found: " + user);
	...
	public boolean update(User user) {
		...
		Transaction tx = ses.beginTransaction();
		ses.clear();
		String sql = String.format("update system_users set system_user_username='%s', system_user_password='%s', system_user_firstname='%s', system_user_lastname='%s' where system_user_id=%d", user.getUsername(), user.getPassword(), user.getFirstName(), user.getLastName(), user.getId());
